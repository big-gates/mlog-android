...
class NetflixMovieMediator(
    ...
    override suspend fun initialize(): InitializeAction {
        val latestUpdate = movieRoomDataSource.getSyncLog(SyncLogType.Netflix_Movie).updatedAt.toDate().time
        val cacheTime = TimeUnit.MILLISECONDS.convert(1, TimeUnit.DAYS)
        return if(System.currentTimeMillis() >= latestUpdate + cacheTime) {
            movieRoomDataSource.clearNetflixMoviesUpdateSyncLogUpdatedAt()
    ...
    override suspend fun load(
        ...
        try {
            val syncLog = movieRoomDataSource.getSyncLog(SyncLogType.Netflix_Movie)
